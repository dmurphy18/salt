...
def serialize(
    ...
    **kwargs
    ...
    deserializer_name = "{}.deserialize".format(formatter)

    if serializer_name not in __serializers__:
        return {
            "changes": {},
            "comment": "{} format is not supported".format(formatter.capitalize()),
            "name": name,
            "result": False,
        }
    ...
    if merge_if_exists:
        if os.path.isfile(name):
            if deserializer_name not in __serializers__:
                return {
                    "changes": {},
                    "comment": "merge_if_exists is not supported for the {} "
